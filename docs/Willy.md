...
│   ├── run_control.py            ← 显式 /resume、/fork 解析与停止步骤/参数归属校验
...
方案摘要会明确要求用户以文本回复确认。只有最新的待确认方案紧随其后时，回复“运行”“开始运行”“确认运行”或等效的短确认语才会启动流水线；普通讨论、否定语和没有待确认方案时的“运行”不会启动。待确认期间，配置助理按确认、增量修改、方案问题和明确新体系四类意图处理：增量修改会把经过指纹校验的待确认候选、方案摘要和最近对话一并提交给 LLM，重新审计后生成并覆写完整的新方案；方案问题只读回答并保留原方案；明确新体系才会使旧方案失效。任何修改都不会直接写入 `config.json` 或启动工序，必须再次确认；最终冻结发生在确认后的 `start_pipeline()`。启动回执保留该用户回复与精简结果，方案摘要只在生成方案时展示一次。

运行中的“中止流水线”采用服务端两次确认：首次点击只显示“确认中止”，只有第二次点击且后端已写入停止请求后，界面才显示“正在安全停止”。运行助理文本框中的“中止”“暂停”“稍后”或“确认中止”只作为讨论或等待指令，不能终止工程。中止后仅完整 `/resume` 或 `/fork` 会在运行助理调用前进入确定性控制层：前者在原 run 的安全步骤续跑，后者只允许修改停止步骤或之后的既有参数并创建带 `parent_run_id` 的独立子 run；任何普通文本均不具控制权限。
