...
| [`../tests/reports/test_case_catalog.md`](../tests/reports/test_case_catalog.md) | 测试用例台账 | 6 号 | 当前：由收集脚本从实际 pytest 用例生成；ORCA Molden→MOL2 键连接/Mayer 键级、OPLS ITP 命名空间、LigParGen GRO 五列残基字段、净电荷容差边界、结构化日志、Packmol ABI/终态升级回归、归档网关离线回归、配置页依赖预检、客户端部署包运行态隔离、四条 profile 的只读终态证据和用户可见 Prompt 的工程绑定/摘要边界已归入对应分类，重新生成并核验（907 条 pytest、18 条 LLM 场景，共 925 条记录）。Python 3.10 的测试 extras 以 `tomli` 兼容发布门禁的 TOML 解析。 | 2026-08-13 | pytest 收集结果、LLM eval 场景、测试对象或测试方式变化。 |
...
| [`project_gap_analysis.md`](project_gap_analysis.md) | 问题台账 | 0 号、5 号，领域负责人认领 | 当前，长期维护：以发布 `v0.4.0` 为基线，登记当前 LLM 协议和真实工具闭环缺口 G-05/G-09；已确定仅支持 GitHub 源码检出，G-00 关闭；T-03 已具备四个具名 profile 的只读脱敏证据契约，G16 + Sobtop 已在受控验收机真实完成十步；新 Ubuntu 22.04 WSL 已安装 ORCA/GROMACS，可继续验收 ORCA + Sobtop，三条剩余路线及标准化归档仍开放。G-02 明确保留仓内 Sobtop、精简 Open Babel 和 3Dmol 的第三方授权证据缺口。本地 mock 和服务端契约已通过，真实 BYOK 报告按当前 16/18 口径留存。明确唯一真人开发者负责 AI 严格无法验证的真实环境矩阵；0-7 号均为受限 AI 协作角色。G09、科学体系预测、后处理/分析与离子 OPLS 均登记为本版本非目标。 | 2026-08-13 | 缺口修复、责任/严重度变化、外部依赖兼容性、稳定基线更新或发布前。 |
