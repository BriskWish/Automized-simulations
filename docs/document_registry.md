...
| [`environment_registry_design.md`](environment_registry_design.md) | 外部运行环境设计 | 0 号 | 当前，长期维护：核心 registry、执行模块依赖归属、模块预检、子进程注入，以及表单值驱动、脱敏且受限的 OpenAI-compatible LLM 连通性检查已实现。LLM 连接请求使用自动工具选择，仍严格验证指定工具调用；独立协议探针区分网络、鉴权、模型工具协议和 provider `required` 兼容性。配置页新增不阻塞任务提交的分组依赖预检，会检查外部工具与内置 Vendor、仅对自动发现的可用外部工具补写缺失 `WILLY_*` 默认项，并保留运行期强制模块预检；记录了 Python/外部工具版本基线、GROMACS 计划验收范围和当前环境漂移。 | 2026-08-12 | 外部工具、vendor 清单、许可证/来源、LLM 或软件环境变量优先级、预检、子进程环境或脱敏运行报告变化。 |
...
| [`revision_strategy.md`](revision_strategy.md) | 总体计划 | 0 号 | 执行中：Phase 1 契约、运行审计、进程生命周期、fixture 完整性和 CI 质量门禁已完成；Agent 恢复已收敛为服务端 DIAGNOSE/RECOVER 状态机，配置生成已分段审计，评分只接受实际工具契约。真实模型协议与 fake-tool 闭环仍为独立验收项。四条 G16/ORCA + Sobtop/LigParGen profile 已有完整十步成功证据；科学体系预测、后处理和 G09 全链路不属于本版本目标。 | 2026-08-12 | Phase 状态、环境/运行注册边界、优先级或完成定义变化。 |
...
| [`testing_strategy.md`](testing_strategy.md) | 验收计划 | 6 号 | 当前，长期维护：2026-08-11 四条最小 profile（G16/ORCA + Sobtop/LigParGen）均完成 10/10；18 个本地 mock 恢复场景全部通过；2026-08-12 当前 BYOK 协议探针 3/3、配置 Agent 5/5 通过，真实恢复矩阵 17/18，`q_scf_001` 的 `model_no_tool_call` 保留为 G-09 稳定性缺口。`md__202608110001` 的 Packmol ABI 问题已有兼容构建和最小 smoke 修复，更新目标机完整链路复测仍待完成。本版本成熟方案以十步完成和最终无错误为准；G09、科学体系预测和后处理/分析已明确排除。 | 2026-08-12 | 测试基线、marker、ETA、smoke 证据、LLM eval、受管进程生命周期、外部运行时 ABI、非当前产品验收矩阵或真实 profile 结果变化。 |
| [`../tests/reports/test_case_catalog.md`](../tests/reports/test_case_catalog.md) | 测试用例台账 | 6 号 | 当前：由收集脚本从实际 pytest 用例生成；ORCA Molden→MOL2 键连接/Mayer 键级、OPLS ITP 命名空间、LigParGen GRO 五列残基字段、净电荷容差边界、结构化日志、Packmol ABI/终态升级回归、归档网关离线回归、配置页依赖预检、客户端部署包运行态隔离和用户可见 Prompt 的工程绑定/摘要边界已归入对应分类，重新生成并核验（872 条 pytest、18 条 LLM 场景，共 890 条记录）。 | 2026-08-12 | pytest 收集结果、LLM eval 场景、测试对象或测试方式变化。 |
...
| [`project_gap_analysis.md`](project_gap_analysis.md) | 问题台账 | 0 号、5 号，领域负责人认领 | 当前，长期维护：以待发布 `v0.3.0` 为基线，登记当前 LLM 协议和真实工具闭环缺口 G-05/G-09；本地 mock 和服务端契约已通过，真实 BYOK 报告按当前口径留存。明确唯一真人开发者负责 AI 严格无法验证的真实环境矩阵；0-7 号均为受限 AI 协作角色。G09、科学体系预测、后处理/分析与离子 OPLS 均登记为本版本非目标。 | 2026-08-12 | 缺口修复、责任/严重度变化、外部依赖兼容性、稳定基线更新或发布前。 |
