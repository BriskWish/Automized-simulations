...
> 本文件是项目文档用途、责任人和生命周期状态的唯一权威台账。`docs/README.md` 只负责导航；设计细节只保留在各主题文档中。最后核验：2026-08-14（初始质量密度默认值 0.7 g/cm3、统一 MD 默认时间步长 0.001 ps、显式 Packmol PBC、实际盒审计与残留输出隔离、私有 Packmol 执行证据、当前 run 隔离、统一 versioned run metadata、运行 provenance、RunStore、受管 GROMACS/量子/拓扑子进程生命周期、StepRegistry、执行模块依赖归属、完整配置外层 schema、四条 profile 的归档验收并集、目标机单链路口径、真实 LLM 的 EQ 参数调整提案和允许重试判断，以及中止后的显式 `/resume`、`/fork` 已同步）。
...
| [`run_assistant_design.md`](run_assistant_design.md) | 运行计划 | 0 号、3 号、4 号 | 执行中：MD 知识库受限检索已完成并转入长期维护；运行目录冻结并以统一 metadata 登记 G16/G09 `.gjf` / ORCA `.inp` 原始输入及可复用中间产物；运行助理的环境查询读取 provenance section 并兼容历史报告；运行与方案助理复用版本化九段 Prompt 契约，运行助理逐轮绑定当前 `run_id`，不传递聊天历史、完整配置、日志尾部或产物清单；当前会话的欢迎语、状态快照、错误、待确认方案和终态升级建议均为独立气泡；输入 `/` 才显示的本地 slash menu 只补全 `/resume`、`/fork` 文本；用户中止后只有完整命令会在只读 Agent 之前走确定性恢复路径；项目架构与设计问答已并入方案助理的白名单文档只读子模式。 | 2026-08-14 | run 数据契约、实时 ETA、性能、经验检索、权限、控制动作、Prompt 契约、项目说明问答或任一 Phase 验收变化。 |
| [`testing_strategy.md`](testing_strategy.md) | 验收计划 | 6 号 | 当前，长期维护：四条最小 profile 的只读终态、阶段许可、固定产物哈希和 JUnit 已归档为历史验收并集；目标机只需一条完整 profile。Chromium fake-executor 浏览器 E2E 已通过 `/resume` 菜单、确认、停止和工程切换；中止后 resume/fork 的解析、状态流转、快照隔离和审计已有本地回归。本版本成熟方案以十步完成、最终无错误和受控 LLM 错误处理为准；科学体系预测和后处理/分析已明确排除。 | 2026-08-14 | 测试基线、marker、ETA、smoke 证据、LLM eval、受管进程生命周期、外部运行时 ABI、非当前产品验收矩阵或真实 profile 结果变化。 |
| [`../tests/reports/test_case_catalog.md`](../tests/reports/test_case_catalog.md) | 测试用例台账 | 6 号 | 当前：由收集脚本从实际 pytest 用例生成；ORCA Molden→MOL2 键连接/Mayer 键级、OPLS ITP 命名空间、LigParGen GRO 五列残基字段、净电荷容差边界、结构化日志、Packmol ABI/终态升级回归、G-07/T-07 WSL 错误矩阵、归档网关离线回归、配置页依赖预检、客户端部署包运行态隔离、四条 profile 的只读终态证据、用户可见 Prompt 的工程绑定/摘要边界，以及中止后受控 resume/fork 已归入对应分类，重新生成并核验（926 条 pytest、18 条 LLM 场景，共 944 条记录）。Python 3.10 的测试 extras 以 `tomli` 兼容发布门禁的 TOML 解析。 | 2026-08-14 | pytest 收集结果、LLM eval 场景、测试对象或测试方式变化。 |
...
| [`project_gap_analysis.md`](project_gap_analysis.md) | 问题台账 | 0 号、5 号，领域负责人认领 | 当前，长期维护：四条历史 profile 的核心 JSON/JUnit 已归档为验收并集，G-03 只跟踪目标机单链路；G-06 的通用问答/历史保留与 G-07 目标机日志复测仍开放。明确唯一真人开发者负责 AI 严格无法验证的真实环境矩阵；0-7 号均为受限 AI 协作角色。科学体系预测、后处理/分析与离子 OPLS 均登记为本版本非目标。 | 2026-08-14 | 缺口修复、责任/严重度变化、外部依赖兼容性、稳定基线更新或发布前。 |
