...
OUTPUT = ROOT / "docs" / "test_case_catalog.md"
...
CATEGORIES = {
    "A": {
        "title": "UI 与前端交互",
        "files": {"test_app_ui.py", "test_frontend_api.py"},
        "description": "Gradio 布局、确认式操作、可视化、状态展示和公开错误边界。",
    },
    "B": {
        "title": "环境与基础错误模型",
        "files": {"test_env_checker.py", "test_env_registry.py", "test_errors.py"},
        "description": "依赖发现、环境变量优先级、能力报告脱敏与结构化错误协议。",
    },
    "C": {
        "title": "配置与全局工具",
    ...
    "D": {
        ...
        "files": {
            ...
            "test_llm_budget.py", "test_llm_config.py",
        },
        "description": "OpenAI-compatible 配置、LayerAgent 的上下文、受控动作、预算、重试、升级，以及 18 个离线 mock LLM 场景。",
    },
    "E": {
        "title": "量子与跨层 Tool 契约",
        "files": {"test_log_parsers.py", "test_toolist_quantum_topology.py"},
        "description": "量子/引擎日志解析、量子工具与跨层 tool 返回协议。",
    },
    "F": {
        "title": "拓扑后端与组装",
        "files": {"test_top_assembly.py", "test_topology_contract.py"},
    ...
    "I": {
        ...
        "files": {
            ...
            "test_run_assistant.py", "test_run_provenance.py", "test_run_store.py",
        },
        "description": "运行预留、互斥启动、进程生命周期、保留清理、审计、provenance、RunStore 与只读助理。",
    },
    "J": {
        "title": "外部 Smoke 与发布证据",
        "files": {"test_external_smoke.py"},
        "description": "外部工具预检、fixture 完整性、证据归档和 required 发布门禁。",
    },
}
...
def _source_index(file_name: str) -> dict[tuple[str, str], tuple[str, str]]:
...
def _pytest_rows(node_ids: list[str]) -> dict[str, list[tuple[str, str, str, str]]]:
    ...
    for number, node_id in enumerate(node_ids, start=1):
        ...
        if file_name not in cache:
            cache[file_name] = _source_index(file_name)
        docstring, source = cache[file_name].get((class_name, function_name), ("", ""))
...
def main() -> None:
    ...
    print(f"Wrote {OUTPUT.relative_to(ROOT)}: {len(node_ids)} pytest + {len(llm_rows)} LLM = {total_rows} records")
