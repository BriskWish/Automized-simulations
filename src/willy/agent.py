"""
agent.py — LLM 交互核心: NL→config 解析、对话管理、方案总结。
...
from willy.knowledge_tools import TOOLS, handle_tool_call
...
def _tool_summary(tool_name: str, result_str: str) -> tuple[str, str]:
    ...
    elif tool_name == "resolve_compound":
        if "error" in r:
            return ("❌", f"{r['error']}")
        return ("🧩", r.get("note", "已拆分"))
    elif tool_name == "lookup_md_defaults":
        return ("⚙️", f"默认: T={r.get('ref_t','?')}K, dt={r.get('dt','?')}ps")
    elif tool_name == "get_box_density":
        return ("📐", f"推荐密度: {r.get('density','?')} 分子/nm³")
    elif tool_name == "lookup_basis_set":
        return ("🔬", f"推荐基组: {r.get('recommended','?')}")
    elif tool_name == "refresh_structs":
        return ("🔄", f"已刷新, {r.get('count','?')} 个分子可用")
    elif tool_name in ("diagnose_error", "diagnose_config_error"):
...
def launch_pipeline() -> str:
    ...
    threading.Thread(target=lambda: os.system(
        f"cd {ROOT} && python3 run_pipeline.py 2>&1"), daemon=True).start()
...
def get_system_prompt() -> str:
    return """你是 MD 配置生成助手。必须用工具查询信息（不用猜），用工具后只输出 JSON。
...
```
- molecules/residues 的 key 必须用 struct/ 下的精确文件名: """ + ", ".join(_available_residues().keys()) + """。用户写 Li+/Li⁺/锂离子 都映射到 Li，NO3-/NO₃⁻/硝酸根 都映射到 NO3
- md 中温度用整数 K（常温/室温=298），时间 ns
- 未指定的参数用 lookup_md_defaults 获取默认值
- 化合物用 resolve_compound 拆分后再查电荷
- **"各"分配**: "A和B各N个"→A=N,B=N; "A、B各N"→都N; 多个"各"依次分配
- **"不加盐""纯溶剂"**: 盐=阴阳离子对, 不加盐=residues只含溶剂分子

## 查询顺序
1. 提取所有分子名 → 逐个 lookup_molecule
2. 未找到 → refresh_structs → 重试 (最多 2 轮)
3. 仍失败 → 返回 Error `{"error":{"type":"invalid_molecule",...}}`
4. 全部存在 → resolve_compound (如有) → lookup_md_defaults (1次) → 计算 total, net charge → 输出 JSON
...
## Few-shot
**Normal**: Li 100, TFSI 100, FEC 300 → {"error":null,"warnings":[],"residues":{"Li":100,"TFSI":100,"FEC":300},...}
...
可用分子: """ + ", ".join(_available_residues().keys())
