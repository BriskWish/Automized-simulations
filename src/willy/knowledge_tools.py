"""
knowledge_tools.py — LLM function calling tools + TF-IDF 向量检索
"""
...
_ERRORS = {
    ...
    "grompp_atomtype": "itp 未过滤 [atomtypes] → 运行 top_maker (自动调 itp_reviser)",
    ...
    if tool_name == "lookup_molecule":
        ...
        if args["name"] in _COMPOUNDS:
            return _json.dumps({
                ...
                "hint": f"请调用 resolve_compound('{args['name']}') 拆分为离子",
            }, ensure_ascii=False)
        ...
        return _json.dumps({"symptom": sym, "fix": fix})
