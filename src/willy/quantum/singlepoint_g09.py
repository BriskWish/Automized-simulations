...
singlepoint_g09.py
==================
G09 两步法 Step 2: 高精度单点能 → *_opt.fchk。

.fchk (DZ优化) → 提取坐标 → *_opt.gjf → g09 SP(def2TZVP) → formchk → *_opt.fchk
"""
...
def run(
    ...
    if opt_fchk.exists():
        print(f"[sp_g09] ⏭ {name}: {opt_fchk.name} 已存在，跳过 G09 SP")
        return StepResult(
            step_name="sp_g09", step_index=2, success=True,
...
"""
    ...
    print(f"[sp_g09] {name}: *_opt.gjf 已生成  (basis={SP_BASIS})")

    try:
        g09 = require_tool("g09")
        formchk = require_tool("g09_formchk")
        g09_env = build_tool_env("g09")
        formchk_env = build_tool_env("g09_formchk")
    ...
    try:
        ...
        return StepResult(
            ...
            error=StepError(kind=ErrorKind.TIMEOUT,
                            message=f"{name}: G09 SP 超时 (3600s)"),
            duration_s=_time.time() - _start,
        )
    ...
    if "Normal termination" not in result.stdout:
        return StepResult(
            ...
            error=StepError(kind=ErrorKind.GAUSSIAN_CRASH,
                            message=f"{name}: G09 SP 未正常终止",
                            raw_output=result.stdout[-500:]),
            duration_s=_time.time() - _start,
        )
    print(f"[sp_g09] {name}: ✅ G09 SP 完成")
    ...
    print(f"[sp_g09] {name}: ✅ {opt_fchk.name}  ({opt_fchk.stat().st_size} bytes)")
