...
def check_env_ready() -> list[str]:
    """预检 g09 / G09 formchk 是否可用。委托到 env_checker。"""
    from willy.env_checker import check_module
    return check_module("struct_g09").failed_strs()
...
def run_one(name: str, cfg: dict, defaults: dict,
    ...
    """
    对单个分子: 修改 .gjf → 跑 g09 → formchk → 输出 .fchk。
    若 .fchk 已存在则跳过。
    ...
    if fchk_path.exists():
        ...
        return StepResult(
            step_name="struct_g09", step_index=1, success=True,
    ...
    if not gjf_path.exists():
        return StepResult(
            step_name="struct_g09", step_index=1, success=False,
    ...
    try:
        ...
        return StepResult(
            ...
            error=StepError(kind=ErrorKind.TIMEOUT,
                            message=f"{name}: G09 结构优化超时 (7200s)"),
            duration_s=time.time() - t0,
        )
