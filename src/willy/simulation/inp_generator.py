...
组分来源统一从 config.json 的 residues 读取，与 top_maker 保持一致。
"""
...
class InpGenerator:
    ...
    def write(self, path: Optional[str] = None) -> Path:
        ...
        print(f"[inp_generator] 已写入: {self._inp_path}")
    ...
    def run(self, inp_path: Optional[str] = None) -> StepResult:
        ...
        try:
            ...
            return StepResult(
                step_name="inp_generator", step_index=7, success=False,
        ...
        if result.returncode != 0:
            ...
            print(f"[inp_generator] ❌ packmol 出错:")
            print(result.stderr)
            return StepResult(
                step_name="inp_generator", step_index=7, success=False,
        ...
        if out_path.exists():
            ...
            print(f"[inp_generator] ✅ 盒子已生成: {out_path} ({size_kb:.1f} KB)")
        print(result.stdout)

        duration = _time.time() - _start
        return StepResult(
            step_name="inp_generator", step_index=7, success=True,
...
# ============================================================
# 从 config.json 自动加载（与 top_maker 同源）
# ============================================================
...
def _gro_to_pdb(gro_path: str, pdb_path: str) -> Path:
    ...
    out = Path(pdb_path)
    if out.exists():
        print(f"[inp_generator]   gmx editconf: {Path(gro_path).name} → {out.name}")
    return out
        ...
        if not pdb_path.exists():
            if gro_path.exists():
                ...
                raise FileNotFoundError(
                    ...
                    f"请先运行 sobtop_interface"
                )

        components.append(Component(pdb=str(pdb_path), count=count))

    print(f"[inp_generator] 从 config.json 加载 {len(components)} 个组分")
