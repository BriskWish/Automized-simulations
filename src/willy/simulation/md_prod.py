...
用法:
  CLI:  python3 -m willy.simulation.md_prod [work_dir]
  API:  from willy.simulation.md_prod import run_prod, ProdResult
"""
...
def run_prod(work_dir: str = None,
    ...
    if not cwd.exists():
        return StepResult(
            step_name="md_prod", step_index=10, success=False,
    ...
    if not gmx_result.success:
        gmx_result.step_name = "md_prod"
        gmx_result.step_index = 10
        return gmx_result

    return StepResult(
        step_name="md_prod", step_index=10, success=True,
