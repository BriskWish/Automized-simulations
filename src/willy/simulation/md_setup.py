...
用法:
  CLI:  python3 -m willy.simulation.md_setup [target_dir]
  API:  from willy.simulation.md_setup import collect_files
"""
...
def collect_files(target_dir: str = None,
    ...
    if topol.exists():
        ...
        for inc_name in _parse_includes(topol):
            ...
            if src.exists():
                ...
                print(f"[md_setup] ⚠  {inc_name}: 文件不存在，跳过")
    else:
        # topol.top 不存在，尝试从 config.json 推断
        print(f"[md_setup] ⚠  {topol} 不存在，从 config.json 推断所需 .itp")
        config_path = ROOT / "config.json"
        if config_path.exists():
            with open(config_path) as f:
                residues = json.load(f).get("residues", {})
            if residues:
                print(f"[md_setup]   请先运行 top_maker 生成 topol.top")
                print(f"[md_setup]   需要的 .itp: {', '.join(residues.keys())}")
        return out
    ...
    if struct.exists():
        ...
        print(f"[md_setup] ⚠  起始结构 {struct} 不存在")

    print(f"[md_setup] ✅ 已收集 {len(collected)} 个文件到 {out}/")
    for f in collected:
        print(f"[md_setup]     {f}")
