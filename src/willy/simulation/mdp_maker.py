...
def build_all(config_path: str = "config.json",
              ...
              overrides: dict = None) -> StepResult:
    """
    ...
    try:
        ...
        return StepResult(
            step_name="mdp_maker", step_index=6, success=False,
    ...
    artifacts: list[str] = []
    for stage, (fname, builder) in _BUILDERS.items():
        ...
        print(f"[mdp_maker] ✅ {fname}  (nsteps → {nsteps})")

    duration = _time.time() - _start
    print(f"[mdp_maker] 完成: {len(results)} 个 .mdp → {out}/  ({duration:.1f}s)")
    return StepResult(
        step_name="mdp_maker", step_index=6, success=True,
