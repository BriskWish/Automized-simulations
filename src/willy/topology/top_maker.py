...
流程:
  ...
  4. 写出 .top 文件
  5. 直接调用 itp_reviser 清理 itp 中的 [atomtypes] 段
"""
...
def _collect_dedup_atomtypes(residues: list[str], topo_dir: str = "topo") -> list[str]:
    ...
    for name in residues:
        ...
        if not itp_path.exists():
            print(f"[top_maker] ⚠  {itp_path} 不存在，跳过 atomtype 提取")
            continue
        ...
        for entry in _extract_atomtypes(itp_path):
            ...
            if at_name not in seen:
                seen.add(at_name)
                result.append(entry)
                print(f"[top_maker]   + {at_name} (from {name})")
            else:
                print(f"[top_maker]   - {at_name} (重复，跳过)")
...
def generate_top(config: TopConfig = None,
    ...
    out.write_text("\n".join(lines))
    print(f"[top_maker] ✅ 主拓扑已生成: {out}")
    print(f"[top_maker]    包含 {len(atomtype_lines)} 个 atomtype, "
...
def build(config_path: str = None,
    ...
    try:
        ...
        return StepResult(
            step_name="top_maker", step_index=5, success=False,
    ...
    if missing:
        ...
        return StepResult(
            ...
            error=StepError(kind=ErrorKind.UNKNOWN,
                            ...
                            raw_output="\n".join(missing),
                            hint="检查 itp 文件是否完整，重新运行 sobtop_interface"),
            artifacts=[str(top_path)],
            duration_s=_time.time() - _start,
        )
    else:
        print("[top_maker] ✅ 结构检查通过")

    # ── Step 3: 直接调用 itp_reviser ──
    print("[top_maker] 调用 itp_reviser 修订 itp…")
    from willy.topology.itp_reviser import revise_all
    count = revise_all(str(TOPO_DIR))
    print(f"[top_maker] ✅ itp_reviser 完成: {count} 个文件修订")

    duration = _time.time() - _start
    return StepResult(
        step_name="top_maker", step_index=5, success=True,
